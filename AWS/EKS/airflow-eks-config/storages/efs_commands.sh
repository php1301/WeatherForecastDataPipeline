...
# before
TAG1=tag:kubernetes.io/role/elb
TAG2=tag:kubernetes.io/role/internal-elb

subnets=$(aws ec2 describe-subnets --filters "Name=$TAG1,Values=1" | jq --raw-output '.Subnets[].SubnetId')
...
subnets=$(aws ec2 describe-subnets --filters "Name=$TAG2,Values=1" | jq --raw-output '.Subnets[].SubnetId')
