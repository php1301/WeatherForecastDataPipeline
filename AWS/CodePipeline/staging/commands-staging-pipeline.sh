...
# deploy the stack
aws cloudformation update-stack --stack-name=airflow-staging-pipeline --template-body=file://WeatherForecastDataPipeline/AWS/CodePipeline/staging/airflow-staging-pipeline.cfn.yml --parameters ParameterKey=EksClusterName,ParameterValue=airflow ParameterKey=KubectlRoleName,ParameterValue=AirflowCodeBuildServiceRole ParameterKey=GitHubUser,ParameterValue=php1301 ParameterKey=GitHubToken,ParameterValue= ParameterKey=GitSourceRepo,ParameterValue=wfdp-docker-eks ParameterKey=GitBranch,ParameterValue=staging